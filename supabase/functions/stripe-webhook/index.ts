...
async function handleEvent(event: Stripe.Event) {
  ...
  if (isSubscription) {
    ...
    try {
      ...
      // Insert into stripe_orders table
      ...
      // Update the app order status if order_id was passed in metadata
      ...
      if (orderId) {
        ...
        if (updateError) {
          ...
          console.info(`Order ${orderId} marked as paid`);
        }
        ...
        if (orderData) {
          await supabase.from('payments').insert({
            order_id: orderId,
            payer_id: orderData.advertiser_id,
            amount: orderData.total_budget,
            service_fee: orderData.total_budget * 0.15,
